...
const columns = [{
    ...
    dataIndex: 'title',
    sorter: true,
    render: (text, entity: PackageViewModel) => {
        return <NavLink to={`/website/admin/package/update?${nameof<PackageViewModel>(o => o.id)}=${entity.id}`}>{text}</NavLink>
    }
}]

class PackageTableComponent extends React.Component<StateProps & DispatchProps> {
    selectedIds = []
    filters = {}
    ...
    @autobind
    onSelectChange(selectedRowKeys, selectedRows: Array<PackageViewModel>) {
        if (!selectedRows)
            return

        this.selectedIds = selectedRows.map(o => o.id)
    }
...
const mapStateToProps = (state: ModuleRootState): StateProps => {
    return {
        dataSource: state.data.getIn(['PACKAGE', 'response']),
        dataSourceProcessing: state.data.getIn(['PACKAGE', 'status', 'processing']),
        deleteResult: state.data.getIn(['PACKAGE_DELETE', 'response', 'result']),
        deleteProcessing: state.data.getIn(['PACKAGE_DELETE', 'status', 'processing'])
    }
}
...
const mapDispatchToProps = (dispatch): DispatchProps => {
    const getDataSource = () => {
        ...
        const requestSendAction = RequestSend('PACKAGE', {
    ...
    return {
        getDataSource,
        selectedDelete: (ids) => {
            const requestSendAction = RequestSend('PACKAGE_DELETE', {
                url: `/package/delete`,
                requestInit: {
                    method: 'DELETE',
                    headers: new Headers({
...
const PackageTableWithPureData = ExtractImmutableHOC(PackageTableComponent)
export const PackageTable = connect(mapStateToProps, mapDispatchToProps)(PackageTableWithPureData)