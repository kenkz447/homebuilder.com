...
const columns = [{
    ...
    dataIndex: 'title',
    sorter: true,
    render: (text, entity: BlogViewModel) => {
        return <NavLink to={`/website/admin/blog/update?${nameof<BlogViewModel>(o => o.id)}=${entity.id}`}>{text}</NavLink>
    }
}]

class BlogTableComponent extends React.Component<StateProps & DispatchProps> {
    selectedIds = []
    filters = {}
    ...
    @autobind
    onSelectChange(selectedRowKeys, selectedRows: Array<BlogViewModel>) {
        if (!selectedRows)
            return
...
const mapStateToProps = (state: ModuleRootState): StateProps => {
    return {
        dataSource: state.data.getIn(['BLOG', 'response']),
        dataSourceProcessing: state.data.getIn(['BLOG', 'status', 'processing']),
        deleteResult: state.data.getIn(['BLOG_DELETE', 'response', 'result']),
        deleteProcessing: state.data.getIn(['BLOG_DELETE', 'status', 'processing'])
    }
}
...
const mapDispatchToProps = (dispatch): DispatchProps => {
    const getDataSource = () => {
        ...
        const requestSendAction = RequestSend('BLOG', {
    ...
    return {
        getDataSource,
        selectedDelete: (ids) => {
            const requestSendAction = RequestSend('BLOG_DELETE', {
                url: `/blog/delete`,
                requestInit: {
                    method: 'DELETE',
                    headers: new Headers({
...
const BlogTableWithPureData = ExtractImmutableHOC(BlogTableComponent)
export const BlogTable = connect(mapStateToProps, mapDispatchToProps)(BlogTableWithPureData)